...
def fetch_meters_location(meterbyarea: MeterByArea) -> MeterIDs:
	####################################################################################################################
	...
	locations = meters_data.get('Location', {})
	####################################################################################################################
	#  MOCK DATASPACE - END                                                                                            #
	####################################################################################################################

	rec_location = (meterbyarea.rec_location.latitude, meterbyarea.rec_location.longitude)
	radius = meterbyarea.radius
...
def fetch_dataspace(user_params: Union[SizingInputs, SizingInputsWithShared]) \
	...
	if hasattr(user_params, 'shared_meter_id'):
		meter_ids.add(user_params.shared_meter_id)
